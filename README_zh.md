...
应用采用 **Material 3** 设计，使用自定义音频播放引擎，支持 **Rubberband** 变调不变速技术。
...
- **下载** 你喜欢的单集，支持离线收听。
- **发现** 新播客 — 在 *Discover* 标签页浏览 *(由 Apple Podcasts 提供支持)*，订阅时有加载反馈。
...
| 音频播放 | 自定义播放器 (Java Sound API + Rubberband + JLayer + JAADec) |
| 图片加载 | Coil 3 |
| 数据库 | JDBC SQLite |
| 网络 | Ktor |
| RSS 解析 | rssparser |
